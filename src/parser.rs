...
pub fn load_rustdoc(
    ...
    match format_version {
        ...
        #[cfg(feature = "v61")]
        61 => {
            let rustdoc: trustfall_rustdoc_adapter_v61::Crate =
                super::parse_or_report_error(path, &file_data, format_version)?;
            match package {
                Some(package) => Ok(VersionedStorage::V61(
                    trustfall_rustdoc_adapter_v61::PackageStorage::from_rustdoc_and_package(
                        rustdoc, package,
                    ),
                )),
                None => Ok(VersionedStorage::V61(
                    trustfall_rustdoc_adapter_v61::PackageStorage::from_rustdoc(rustdoc),
                )),
            }
        }
